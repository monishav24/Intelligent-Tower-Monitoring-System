"""
Flask REST API Blueprint module exposing JSON telemetry endpoints for dashboard, system health, diagnostic info, and demo mode controls.
"""
import os
import sys
...
@api_bp.route("/api/telemetry/latest", methods=["GET"])
def get_latest():
    """Retrieve full latest telemetry snapshot directly from newest SQLite row."""
...
@api_bp.route("/api/telemetry/history", methods=["GET"])
def get_history():
    """Retrieve N latest time-series points in chronological order for Chart.js rendering."""
...
@api_bp.route("/api/telemetry/diagnostic", methods=["GET"])
@api_bp.route("/api/diagnostic", methods=["GET"])
def get_diagnostic():
    """Retrieve diagnostic telemetry pipeline status (row_count, timestamps, seconds_since_latest)."""
    diag = db.get_telemetry_diagnostic()
    return jsonify({
        "status": "success",
        "diagnostic": diag
    })
...
def get_network_metrics():
    ...
    return jsonify({
        ...
        "traffic_mb": snapshot.get("traffic"),
        "delay_ms": snapshot.get("delay"),
        "throughput_mbps": snapshot.get("throughput"),
        "propagation_time_ms": snapshot.get("propagation_time"),
...
def get_prediction():
    """Retrieve predictions for all 5 ML parameters generated by the fixed selected model."""
    snapshot = db.get_latest_snapshot()
    if not snapshot:
        return jsonify({"status": "no_data"}), 200

    selected_model = snapshot.get("selected_model") or snapshot.get("best_algorithm") or snapshot.get("active_model", "Random Forest")
    ...
    return jsonify({
        ...
        "predictions": {
            "traffic_mb": snapshot.get("predicted_traffic", snapshot.get("predicted_network_traffic", 0.0)),
            "delay_ms": snapshot.get("predicted_delay", 0.0),
            "throughput_mbps": snapshot.get("predicted_throughput", 0.0),
            "propagation_time_ms": snapshot.get("predicted_propagation_time", 0.0),
            "ram_usage_pct": snapshot.get("predicted_ram_usage", 0.0)
        },
        "congestion_risk_pct": snapshot.get("congestion_risk", 0.0),
        "predicted_status": snapshot.get("predicted_status", "NORMAL"),
        "model_type": f"Selected Model: {selected_model}"
    })
...
def receive_esp32_telemetry():
    """
    Receive real hardware telemetry payload from ESP32 IoT node (Temperature Sensor).
    ...
    return jsonify({
        "status": "success",
        "message": "ESP32 temperature telemetry processed successfully",
        "overall_status": overall,
        "control": {
            "green_led": green_led,
            "red_led": red_led,
...
def get_esp32_status():
    """Retrieve ESP32 IoT hardware node connectivity and latest temperature sensor metric."""
    if monitoring_service_instance and hasattr(monitoring_service_instance, "esp32_collector"):
