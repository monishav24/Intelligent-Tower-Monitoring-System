...
def get_prediction():
    """Retrieve predictions for all 5 ML parameters generated by the active winning model."""
    snapshot = db.get_latest_snapshot()
    if not snapshot:
        return jsonify({"status": "no_data"}), 200

    active_model = snapshot.get("active_model", "Random Forest")
    ...
    return jsonify({
        ...
        "model_type": f"Active Model: {active_model}"
    })
